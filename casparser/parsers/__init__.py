...
def read_cas_pdf(
    ...
    """
    Parse CAS pdf and return line data.
    
    :param file: Uploaded CAS pdf file (CAMS or Kfintech)
    :param password: CAS pdf password
    :param output: Output format (json, dict) [default: dict]
    :param sort_transactions: Sort transactions by date and re-compute balances.
    :param force_pdfminer: Force pdfminer parser even if mupdf is detected
    ...
    try:
        ...
        partial_cas_data = cas_pdf_to_text(io.BytesIO(content), password)
        ...
        return {
            "error": "Error parsing pdf file. Please check if the file is a CAS pdf file and if the password is correct."
        }